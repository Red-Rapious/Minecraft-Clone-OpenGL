...
World::World(GLFWwindow* window, glm::vec3 defaultCameraPosition)
	: m_control(window, defaultCameraPosition), m_blocksShader("res/shaders/Blocks.shader"), m_basicShader("res/shaders/Basic.shader"), m_blocksTexture("res/textures/default_mc_textures.png"), m_text("res/textures/ascii.png"), m_crossair("res/textures/crossair.png")
{
	m_vao.Bind();
...
void World::UpdateFrame(GLFWwindow* window)
{
    ...
    {
        m_basicShader.Bind();
        m_basicShader.SetUniform4f("u_Color", 1.0, 1.0, 1.0, 1.0); // set text color
        {
            // Camera position
            ...
            m_text.PrintText(window, m_vao, &m_basicShader, "Camera position : x = " + x + " y = " + y + " z=" + z, 10, 10, m_textSize);
        }

        // Player in chunk position
        ChunkCoord chunkCoord = m_map.GetPlayerPosition();
        m_text.PrintText(window, m_vao, &m_basicShader, "Actual chunk: x = " + std::to_string(chunkCoord.idx) + " z=" + std::to_string(chunkCoord.idz), 10, 10 + m_textSize, m_textSize);

        // OpenGL version
        m_text.PrintText(window, m_vao, &m_basicShader, m_openGLVersion, m_textSize / 2, 745, m_textSize);
        ...
        if (fps < 30)
        {
            m_basicShader.SetUniform4f("u_Color", 1.0, 0.2, 0.20, 1.0);
        }
        else if (fps > 60)
        {
            m_basicShader.SetUniform4f("u_Color", 0.2, 1.0, 0.33, 1.0);
        }
        m_text.PrintText(window, m_vao, &m_basicShader, "Delta = " + std::to_string((int)(spf * 1000)) + "ms   FPS=" + std::to_string((int)fps), 10, 10 + 2 * m_textSize, m_textSize);
